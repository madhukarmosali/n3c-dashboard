...
function localHorizontalStackedBarChart(data, domName, barLabelWidth) {
	...
	var z = d3.scaleOrdinal()
	  ...
	  y.domain(data.map(function(d) { return d.${param.primary_group}; }));					// x.domain...
	  x.domain([0, d3.max(data, function(d) { return d.total; })]).nice();	// y.domain...
	  z.domain(data.map(function(d,i) { return d.${param.secondary_group}; }));
	  ...
	  g.append("g")
	    ...
	    .enter().append("rect")
	      .attr("y", function(d) { return y(d.data.${param.primary_group}); })	    //.attr("x", function(d) { return x(d.data.State); })
