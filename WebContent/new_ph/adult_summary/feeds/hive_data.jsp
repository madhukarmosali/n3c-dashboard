...
<sql:query var="nodes" dataSource="jdbc/N3CPublic">
	select '0-'||age_seq as mapping, age_bin as label, age_abbrev as short_label, 0 as x, (age_seq * 1.0)/(select max(age_seq) from n3c_dashboard.age_map5) as index from n3c_dashboard.age_map5
	union
	select '1-'||severity_seq as mapping, severity as label, severity_abbrev as short_label, 1 as x, (severity_seq * 1.0)/(select max(severity_seq) from n3c_dashboard.severity_map) as index from n3c_dashboard.severity_map
	union
	select distinct '2-'||gender_seq as mapping, gender_abbrev as label, gender_abbrev as short_label, 2 as x, (gender_seq * 1.0)/(select max(gender_seq) from n3c_dashboard.gender_map2) as index from n3c_dashboard.gender_map2
	union
	select '3-'||race_seq as mapping, race as label, race_abbrev as short_label, 3 as x, (race_seq * 1.0)/(select max(race_seq) from n3c_dashboard.race_map) as index from n3c_dashboard.race_map
	union
	select '4-'||ethnicity_seq as mapping, ethnicity as label, ethnicity_abbrev as short_label, 4 as x, (ethnicity_seq * 1.0)/(select max(ethnicity_seq) from n3c_dashboard.ethnicity_map) as index from n3c_dashboard.ethnicity_map
	order by 4,5
</sql:query>
<c:forEach items="${nodes.rows}" var="row" varStatus="rowCounter">
	{ "mapping":"${row.mapping}", "label":"${row.label}", "short_label":"${row.short_label}", "x":${row.x}, "y":${row.index} }<c:if test="${!rowCounter.last}">,</c:if>
</c:forEach>
...
<sql:query var="edges" dataSource="jdbc/N3CPublic">
	...
	select '2-'||race_seq as source, '4-'||ethnicity_seq as target, sum(case when count='<20' then 0 else count::int end) as weight
